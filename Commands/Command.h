...
namespace SocketCommands {
    ...
    public:
        CommandKey(const string &keyRsa, const string &keyAesB64, const string &ivAesB64) : Command(
                COMMAND_KEY_RESPONSE),
                                                                                            key_RSA(keyRsa),
    ...
    public:
        ...
        CommandFetchChat(const vector<Message *> m, string status) : Command(COMMAND_FETCH_CHAT_RESPONSE), m(m),
                                                                     status(status) {}
